...
def ride():
    if request.form:
        ride = Ride(ministry_id=request.form['ministry_id'], dest=request.form['dest'],
                    date=datetime.strptime(request.form['date'], '%m/%d/%y'), driver_name=request.form['driver_name'])
        db.session.add(ride)
        db.session.commit()
    return redirect(url_for('index'))
